  ...
  describe 'constants' do
    describe 'STARTS_WITH_SHOULD' do
      ...
      it 'does not match the word "should" in the middle of a string' do
        expect('foo should bar').not_to match(subject)
      end
      ...
      it "does not match shouldn't in the middle" do
        expect("foo shouldn't bar").not_to match(subject)
      end

      it 'does not other words starting with should' do
        expect('shoulders are nice to have').not_to match(subject)
        expect('shoulda is a helpful library for testing Rails').not_to match(subject)
      end
    end
